    ...
    <div className="min-h-screen bg-[#0f172a] text-slate-100 flex flex-col font-sans selection:bg-teal-500/30">
      ...
      <main className="flex-1 flex flex-col items-center py-16 px-6">
        ...
        <div className="text-center max-w-3xl mb-12 animate-in fade-in slide-in-from-bottom-4">
          ...
          <p className="text-xl text-slate-400">Whether you need a pipe fixed or you are looking for your next job, our pricing is simple, fair, and upfront.</p>
        </div>

        {/* View Toggle */}
        <div className="bg-slate-800/50 p-1.5 rounded-full border border-slate-700 mb-16 flex animate-in fade-in slide-in-from-bottom-6 shadow-2xl">
          <button 
            onClick={() => setView('homeowner')}
            className={`px-8 py-3 rounded-full font-bold transition-all text-sm sm:text-base ${view === 'homeowner' ? 'bg-teal-500 text-slate-900 shadow-md' : 'text-slate-400 hover:text-white'}`}
          >
            For Homeowners
          </button>
          <button 
            onClick={() => setView('trade')}
            className={`px-8 py-3 rounded-full font-bold transition-all text-sm sm:text-base ${view === 'trade' ? 'bg-amber-500 text-slate-900 shadow-md' : 'text-slate-400 hover:text-white'}`}
          >
            For Tradespeople
          </button>
        </div>
